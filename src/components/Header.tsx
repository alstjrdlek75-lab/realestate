...
import { Building2, RefreshCw, Share2, Compass, ShieldCheck, MapPin, Sparkles, BookOpen, History } from 'lucide-react';
...
interface HeaderProps {
  ...
  currentView: 'HERO' | 'DIAGNOSTIC' | 'RESULT' | 'HISTORY';
}
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-1.5 bg-slate-100 p-1 rounded-2xl border border-slate-200 text-xs font-bold">
          <button
            onClick={onReset}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            onClick={onGoToHistory}
            className={`lg:hidden flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold transition border cursor-pointer ${
              isHistoryView 
                ? 'bg-[#edf4ff] text-[#0066ff] border-[#0066ff]/30' 
          ...
          {isResultView ? (
            ...
            <div className="flex items-center gap-3">
              <div className="hidden md:flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                ...
                <span>2026 실거래 데이터</span>
              </div>
              <button
                onClick={onGoToDiagnostic}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#03c75a] hover:bg-[#02b14f] text-white text-xs sm:text-sm font-bold shadow-sm transition-all hover:scale-105 cursor-pointer"
              >
