...
for (const [artId, yt] of Object.entries(YT_MAP)) {
  // Find id: 'artId'
  const idPattern = new RegExp("id:\\s*'" + artId + "'");
  const pos = content.search(idPattern);
  if (pos === -1) {
    console.log("Could not find article id:", artId);
    continue;
  }

  // Find the next `publishedAt:` or `conclusion:`
  // Let's insert `youtubeRecommendation` right after `id: 'artId',`
  const insertionText = `
  ...
  content = content.replace(idPattern, `id: '${artId}',` + insertionText);
}

fs.writeFileSync('src/data/thoughtArticles.ts', content, 'utf-8');
console.log('Fixed! Successfully injected YouTube data into all 34 articles!');
